<?php


namespace View\Site;


use chillerlan\QRCode\QRCode;
use Model\Carteira;
use Pummax\Configuration\DataBase;
use Pummax\View\BaseView;

class FrontEndCarteirinha extends BaseView
{


    public function __construct($createMenu = true, $createFooter = false)
    {
        parent::__construct($createMenu, $createFooter);
    }

    public function createHtml()
    {
        /** @var $carteira Carteira */
        $carteira = $this->getEntityManager()->getRepository(Carteira::class)->findOneBy(['usuario' => $this->getUserModelSession()]);
        if ($carteira) {
            $qrCodeGenerator = new QRCode();
            $qrCode = $qrCodeGenerator->render($carteira->getUsuario()->getPessoa()->getCpf() . $carteira->getDataVencimento()->format('dmY'));
            ?>
            <div class="d-flex justify-content-center">
            <div class="col-sm-12 col-lg-3 col-md-10" style="overflow: auto">
                <div class="id-card-holder">
                    <div class="id-card">
                        <div class="d-flex justify-content-center headers">
                            <div class="col-sm-6">
                                <img src="/utils/img/logo2.jpg">
                            </div>
                            <div class="col-sm-6">
                                <img src="data:image/png;base64,">
                            </div>
                        </div>
                        <div class="photo">
                            <img src="utils/<?= $carteira->getImagem()->getPatch() ?>">
                        </div>
                        <h2>Sócio <?= DataBase::ALIAS_SISTEMA ?></h2>
                        <h5><?= DataBase::NOME_SISTEMA ?></h5>
                        <h6>
                            Nome:&nbsp;<?= $carteira->getUsuario()->getPessoa()->getNome() . " " . $carteira->getUsuario()->getPessoa()->getSobrenome() ?></h6>
                        <h6>
                            Nascimento:&nbsp;<?= $carteira->getUsuario()->getPessoa()->getDataNascimento()->format('d/m/Y') ?></h6>
                        <h6>CPF:&nbsp;<?= $carteira->getUsuario()->getPessoa()->getCpf() ?></h6>
                        <h6>Matrícula:&nbsp;<?= $carteira->getUsuario()->getPessoa()->getMatricula() ?></h6>
                        <h6>Curso:&nbsp;<?= $carteira->getCurso()->getNome() ?></h6>
                        <div class="qr-code">
                            <img src="<?= $qrCode ?>">
                        </div>
                        <br/>
                        <h7>Vencimento:&nbsp;<?= $carteira->getDataVencimento()->format('d/m/Y') ?></h7>
                    </div>
                </div>
            </div>
        <?php } else { ?>
            <div class="alert alert-danger text-center">
                <span>O usuário da sessão atual não possui nenhuma carteirinha</span>
            </div>
        <?php } ?>
        </div>
        <?php
    }
}

?>